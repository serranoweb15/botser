...
async function handleSendScheduledMessage(job) {
  ...
  try {
    ...
    //adicionado enviar anexo nos agendamentos Shedules
    let filePath = null;
        ...
        if (invoice[0]['mycount'] > 0) {
          ...
          const invoiceInsert = await sequelize.query(sql,
           ...
           let transporter = nodemailer.createTransport({
            service: 'gmail',
            auth: {
              user: 'mginformaoficial@gmail.com',
              pass: 'yaptzqmpyeunlqkq'
            }
          });

          const mailOptions = {
            from: 'mginformaoficial@gmail.com', // sender address
            to: `${c.email}`, // receiver (use array of string for a list)
            subject: 'Fatura gerada - Sistema', // Subject line
            html: `Prezado ${c.name}! <br> Gostaríamos de lembrar que sua mensalidade no <b>Multiconnect</b> vence <b>${vencimento}</b> no valor de <b>${plan.value}</B>, não esqueça de realizar o pagamento da sua fatura.<br> Para continuar a utilizar os nossos serviços, pedimos que realize o pagamento na tela "Financeiro" o mais breve possível. Em caso de dúvidas ou para mais informações sobre como realizar o pagamento da fatura, entre em contato conosco através do nosso WhatsApp no telefone (17) 99654-1882.<br>
<br>
<b>Detalhes da Fatura<br></b>
Vencimento: ${vencimento}<br>
Valor: ${plan.value}<br>
Link: ${process.env.FRONTEND_URL}/financeiro<br>
<br>
Qualquer duvida estamos a disposição!<br><br>
Atenciosamente<br>
Equipe Multiconnect<br><br><br> <b>NÃO RESPONDER ESTE E-MAIL AUTOMÁTICO, EM CASO DE DÚVIDAS POR GENTILEZA CONTATAR NOSSO SUPORTE ATRAVÉS DO TELEFONE (17) 99654-1882</b>
            `// plain text body
          };
